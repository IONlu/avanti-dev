...
Vagrant.configure(2) do |config|
    ...
    $script = <<-SCRIPT
        # install locales
        sudo locale-gen fr_FR.UTF-8 de_LU.UTF-8 de_DE.UTF-8
        sudo update-locale
        ...
        # install libapache2-mod-fastcgi (jose fix)
        ...
        # install python for w/e reason (jose fix)
        sudo apt-get update
        ...
        for MODULE in "avanti-core" "avanti-cli" "avanti-api"; do
            ...
            if [ "$MODULE" != "avanti-core" ]; then
                sudo npm link avanti-core --force
            fi
        done
    SCRIPT
